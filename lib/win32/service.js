...
exports.stop = function(key, cb) {
  ...
  run('sc stop ' + key, function(err, out) {
    if (out.toString().match('1062')) { // not running
      ...
      error.code = 'NOT_RUNNING'; // used by satan.ensure_stopped() to check
...
exports.delete = function(key, cb) {
  run(`sc config ${key} start=disabled`, () => {
    run('sc delete ' + key, function(_e, out) {
      if (out?.toString().match('1060')) return cb(new Error(out.toString()))
      cb();
    });
  });
